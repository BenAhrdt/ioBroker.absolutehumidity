/* eslint-disable jsdoc/check-tag-names, jsdoc/require-jsdoc, jsdoc/require-param-description, jsdoc/reject-any-type */
'use strict';

const { DeviceManagement } = require('@iobroker/dm-utils');
const {
	ADAPTER_ICON,
	CARD_HEADER_BACKGROUND,
	OUTDOOR_CARD_HEADER_BACKGROUND,
	STATE_ABSOLUTE_HUMIDITY,
	STATE_DEVICE_MANAGER_HTML,
} = require('./constants');
const { translate, translateToLanguage } = require('./i18n');

const STATE_ICONS = {
	temperature: createSvgIcon(`
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path fill="none" stroke="#1976d2" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M14 14.76V5a4 4 0 0 0-8 0v9.76a6 6 0 1 0 8 0Z"/>
			<path fill="#1976d2" d="M10 17.5a2.5 2.5 0 1 1-2-2.45V8a2 2 0 0 1 4 0v7.05a2.5 2.5 0 0 1-2 2.45Z"/>
		</svg>
	`),
	relativeHumidity: createSvgIcon(`
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path fill="#03a9f4" d="M12 2.7 6.6 9.3A8.2 8.2 0 0 0 5 14a7 7 0 0 0 14 0 8.2 8.2 0 0 0-1.6-4.7L12 2.7Z"/>
			<text x="12" y="16" text-anchor="middle" font-family="Arial, sans-serif" font-size="7" font-weight="700" fill="#fff">%</text>
		</svg>
	`),
	absoluteHumidity: createSvgIcon(`
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path fill="#03a9f4" d="M12 2.7 6.6 9.3A8.2 8.2 0 0 0 5 14a7 7 0 0 0 14 0 8.2 8.2 0 0 0-1.6-4.7L12 2.7Z"/>
			<path fill="rgba(255,255,255,.7)" d="M8.4 14.1c.3 2.1 1.6 3.3 3.4 3.7-2.8.3-5-1.2-5.4-3.6-.2-1.2.2-2.4 1.1-3.5-.1 1.2.2 2.4.9 3.4Z"/>
		</svg>
	`),
	dewPointTemperature: createSvgIcon(`
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path fill="none" stroke="#1976d2" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M10 13.9V5a3 3 0 0 0-6 0v8.9a5 5 0 1 0 6 0Z"/>
			<path fill="#1976d2" d="M7 17.6a2.1 2.1 0 0 1-1-4V8a1 1 0 0 1 2 0v5.6a2.1 2.1 0 0 1-1 4Z"/>
			<path fill="#03a9f4" d="M17 6.4 13.7 10a5 5 0 0 0-1.2 3.2 4.5 4.5 0 0 0 9 0 5 5 0 0 0-1.2-3.2L17 6.4Z"/>
		</svg>
	`),
};

const TEMPERATURE_STATE_FILTER =
	"const unit = String(obj.common?.unit || '').trim().toLowerCase().replace(/\\s+/g, ''); return obj.type === 'state' && obj.common?.type === 'number' && (unit === '' || unit === '°c' || unit === 'c' || unit === '°f' || unit === 'f');";
const RELATIVE_HUMIDITY_STATE_FILTER =
	"const unit = String(obj.common?.unit || '').trim().toLowerCase().replace(/\\s+/g, ''); return obj.type === 'state' && obj.common?.type === 'number' && (unit === '' || unit === '%');";

/**
 * @augments {DeviceManagement<any, string>}
 */
class AbsoluteHumidityDeviceManagement extends DeviceManagement {
	/**
	 * @param {any} adapter
	 */
	constructor(adapter) {
		super(adapter, true);
		this.refreshSequence = 0;
	}

	/**
	 * @returns {import('@iobroker/dm-utils').InstanceDetails} Instance information.
	 */
	getInstanceInfo() {
		return {
			apiVersion: 'v3',
			communicationStateId: `${this.adapter.namespace}.info.deviceManager`,
			actions: [
				{
					id: 'addDevice',
					icon: 'add',
					title: {
						en: '+ Add device',
						de: '+ Gerät hinzufügen',
						ru: '+ Добавить устройство',
						pt: '+ Adicionar dispositivo',
						nl: '+ Apparaat toevoegen',
						fr: '+ Ajouter un appareil',
						it: '+ Aggiungi dispositivo',
						es: '+ Añadir dispositivo',
						pl: '+ Dodaj urządzenie',
						uk: '+ Додати пристрій',
						'zh-cn': '+ 添加设备',
					},
					description: translate('Add a temperature and humidity device'),
					color: CARD_HEADER_BACKGROUND,
					backgroundColor: CARD_HEADER_BACKGROUND,
					variant: 'contained',
					style: {
						backgroundColor: CARD_HEADER_BACKGROUND,
						color: '#ffffff',
					},
					handler: async context => {
						const data = await showDeviceForm(context, undefined);

						if (!data) {
							return { refresh: false };
						}

						await this.adapter.addDevice(data);
						return { refresh: true };
					},
				},
			],
		};
	}

	/**
	 * @param {import('@iobroker/dm-utils').DeviceLoadContext<string>} context
	 */
	async loadDevices(context) {
		const devices = await getDevicesSortedByAbsoluteHumidity(this.adapter);

		context.setTotalDevices(devices.length);

		for (const device of devices) {
			const sourceValidation = await this.adapter.getDeviceSourceValidation(device);

			context.addDevice(createDeviceInfo(this.adapter, device, sourceValidation));
		}
	}

	async refreshDevices() {
		this.refreshSequence += 1;
		// dm-gui-components exposes backend commands, but the Admin JSONConfig wrapper currently
		// does not subscribe to this state. Keep sending it for compatible/future wrappers.
		await this.sendCommandToGui(
			/** @type {any} */ ({
				command: 'all',
				sequence: this.refreshSequence,
			}),
		);
	}
}

/**
 * @param {any} adapter
 * @param {Record<string, any>} device
 * @param {{ valid: boolean; temperatureStateValid: boolean; relativeHumidityStateValid: boolean }} sourceValidation
 * @returns {import('@iobroker/dm-utils').DeviceInfo<string>} Device manager card information.
 */
function createDeviceInfo(adapter, device, sourceValidation) {
	return {
		id: device.id,
		name: device.name,
		icon: ADAPTER_ICON,
		backgroundColor: device.isOutdoor ? OUTDOOR_CARD_HEADER_BACKGROUND : CARD_HEADER_BACKGROUND,
		...(sourceValidation.valid
			? {}
			: {
					status: {
						connection: 'disconnected',
						warning: translate('Invalid source state'),
					},
				}),
		actions: [
			{
				id: 'editDevice',
				icon: 'edit',
				description: translate('Edit device'),
				handler: async (deviceId, context) => {
					const existingDevice = adapter.getDeviceById(deviceId);

					if (!existingDevice) {
						await context.showMessage(translate('Device not found'));
						return { refresh: 'none' };
					}

					const data = await showDeviceForm(context, existingDevice);

					if (!data) {
						return { refresh: 'none' };
					}

					await adapter.updateDevice(deviceId, data);
					return { refresh: 'devices' };
				},
			},
			{
				id: 'removeDevice',
				icon: 'delete',
				description: translate('Delete device'),
				color: 'secondary',
				confirmation: translate('Delete this device?'),
				handler: async deviceId => {
					await adapter.removeConfiguredDevice(deviceId);
					return { refresh: 'devices' };
				},
			},
		],
		customInfo: createDeviceCustomInfo(adapter, device),
	};
}

/**
 * @param {any} adapter
 * @param {Record<string, any>} device
 * @returns {import('@iobroker/dm-utils').DeviceDetails<string>} Device manager custom information.
 */
function createDeviceCustomInfo(adapter, device) {
	return {
		id: device.id,
		schema: {
			type: 'panel',
			innerStyle: {
				marginTop: 0,
				paddingTop: 0,
			},
			items: {
				measurements: {
					type: 'state',
					label: '',
					oid: `${adapter.getDeviceObjectId(device.id)}.${STATE_DEVICE_MANAGER_HTML}`,
					control: 'html',
					readOnly: true,
					size: 'normal',
					narrow: false,
					style: {
						width: '100%',
						minHeight: 200,
						boxSizing: 'border-box',
					},
					newLine: true,
					xs: 12,
					sm: 12,
					md: 12,
					lg: 12,
					xl: 12,
				},
			},
		},
	};
}

/**
 * @param {string} svg
 */
function createSvgIcon(svg) {
	return `data:image/svg+xml;base64,${Buffer.from(svg.replace(/\s+/g, ' ').trim()).toString('base64')}`;
}

/**
 * @param {{ absoluteHumidity: number | null; relativeHumidity: number | null; temperature: number | null; dewPointTemperature: number | null }} values
 * @param {string} language
 * @returns {string} Safe HTML for the four live Device Manager measurements.
 */
function createMeasurementDisplayHtml(values, language) {
	const valueFormatter = new Intl.NumberFormat(language || 'en', {
		maximumFractionDigits: 2,
		useGrouping: false,
	});
	const escapeHtml = value =>
		String(value).replace(/[&<>"']/g, character => {
			switch (character) {
				case '&':
					return '&amp;';
				case '<':
					return '&lt;';
				case '>':
					return '&gt;';
				case '"':
					return '&quot;';
				default:
					return '&#39;';
			}
		});
	const rows = [
		{
			label: translateToLanguage('Absolute humidity', language),
			icon: STATE_ICONS.absoluteHumidity,
			value: values.absoluteHumidity,
			unit: 'g/m³',
		},
		{
			label: translateToLanguage('Relative humidity', language),
			icon: STATE_ICONS.relativeHumidity,
			value: values.relativeHumidity,
			unit: '%',
		},
		{
			label: translateToLanguage('Temperature', language),
			icon: STATE_ICONS.temperature,
			value: values.temperature,
			unit: '°C',
		},
		{
			label: translateToLanguage('Dew point temperature', language),
			icon: STATE_ICONS.dewPointTemperature,
			value: values.dewPointTemperature,
			unit: '°C',
		},
	];

	const rowHtml = rows
		.map(row => {
			const value = Number.isFinite(row.value) ? valueFormatter.format(row.value) : '—';

			return `<span style="display:grid;grid-template-columns:24px minmax(0,1fr) max-content 34px;column-gap:3px;align-items:center;height:44px;min-height:44px;box-sizing:border-box;">
				<img src="${escapeHtml(row.icon)}" alt="" style="display:block;width:24px;height:24px;object-fit:contain;">
				<span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:16px;font-weight:400;line-height:1.2;">${escapeHtml(row.label)}:</span>
				<span style="font-size:16px;font-variant-numeric:tabular-nums;line-height:1.2;white-space:nowrap;">${escapeHtml(value)}</span>
				<span style="width:34px;min-width:34px;box-sizing:border-box;color:rgba(0,0,0,.7);font-size:14px;line-height:1.2;white-space:nowrap;text-align:right;">${escapeHtml(row.unit)}</span>
			</span>`;
		})
		.join('');

	return `<span style="display:flex;flex-direction:column;row-gap:8px;width:264px;max-width:100%;box-sizing:border-box;font-family:Roboto,Arial,sans-serif;">${rowHtml}</span>`;
}

/**
 * @param {any} context
 * @param {Record<string, any> | undefined} device
 */
async function showDeviceForm(context, device) {
	return context.showForm(getDeviceFormSchema(), {
		title: device ? translate('Edit device') : translate('Add device'),
		data: device
			? {
					name: device.name,
					isOutdoor: device.isOutdoor === true,
					temperatureStateId: device.temperatureStateId,
					relativeHumidityStateId: device.relativeHumidityStateId,
					createTemperatureState: device.createTemperatureState,
					createRelativeHumidityState: device.createRelativeHumidityState,
				}
			: {
					name: '',
					isOutdoor: false,
					temperatureStateId: '',
					relativeHumidityStateId: '',
					createTemperatureState: true,
					createRelativeHumidityState: true,
				},
		buttons: ['apply', 'cancel'],
		minWidth: 520,
		applyDisabledRule: '!data.name || !data.temperatureStateId || !data.relativeHumidityStateId',
	});
}

function getDeviceFormSchema() {
	return {
		type: 'panel',
		items: {
			name: {
				type: 'text',
				label: translate('Name'),
				newLine: true,
				xs: 12,
				sm: 12,
			},
			temperatureStateId: {
				type: 'objectId',
				label: translate('Temperature state'),
				newLine: true,
				xs: 12,
				sm: 12,
				customFilter: {
					type: 'state',
					common: {
						type: 'number',
					},
				},
				filterFunc: TEMPERATURE_STATE_FILTER,
			},
			createTemperatureState: {
				type: 'checkbox',
				label: translate('Create temperature state in adapter'),
				newLine: true,
				xs: 12,
				sm: 12,
			},
			relativeHumidityStateId: {
				type: 'objectId',
				label: translate('Relative humidity state'),
				newLine: true,
				xs: 12,
				sm: 12,
				customFilter: {
					type: 'state',
					common: {
						type: 'number',
					},
				},
				filterFunc: RELATIVE_HUMIDITY_STATE_FILTER,
			},
			createRelativeHumidityState: {
				type: 'checkbox',
				label: translate('Create relative humidity state in adapter'),
				newLine: true,
				xs: 12,
				sm: 12,
			},
			isOutdoor: {
				type: 'checkbox',
				label: translate('Outdoor device'),
				newLine: true,
				xs: 12,
				sm: 12,
			},
		},
	};
}

/**
 * @param {any} adapter
 * @param {Record<string, any>[]} devices
 */
async function getDevicesSortedByAbsoluteHumidity(adapter, devices = adapter.getConfiguredDevices()) {
	const devicesWithHumidity = await Promise.all(
		devices.map(async device => ({
			device,
			absoluteHumidity: await readAbsoluteHumidityValue(adapter, device),
		})),
	);

	return devicesWithHumidity
		.sort(
			(left, right) => compareAbsoluteHumidity(left, right) || left.device.name.localeCompare(right.device.name),
		)
		.map(entry => entry.device);
}

/**
 * @param {{ absoluteHumidity: number | null }} left
 * @param {{ absoluteHumidity: number | null }} right
 */
function compareAbsoluteHumidity(left, right) {
	if (left.absoluteHumidity === null && right.absoluteHumidity === null) {
		return 0;
	}

	if (left.absoluteHumidity === null) {
		return 1;
	}

	if (right.absoluteHumidity === null) {
		return -1;
	}

	return left.absoluteHumidity - right.absoluteHumidity;
}

/**
 * @param {any} adapter
 * @param {Record<string, any>} device
 */
async function readAbsoluteHumidityValue(adapter, device) {
	try {
		const state = await adapter.getStateAsync(`${adapter.getDeviceObjectId(device.id)}.${STATE_ABSOLUTE_HUMIDITY}`);
		const value = Number(state?.val);

		return Number.isFinite(value) ? value : null;
	} catch (error) {
		adapter.log.debug(`Cannot read absolute humidity for device "${device.name}": ${error.message}`);
		return null;
	}
}

module.exports = {
	AbsoluteHumidityDeviceManagement,
	createMeasurementDisplayHtml,
	createDeviceInfo,
	getDevicesSortedByAbsoluteHumidity,
	getDeviceFormSchema,
	showDeviceForm,
};
